/**
 * Hands-off E2E QA runner for partner_enrich.
 * Dispatches → polls automation_runs → validates org_extractions.
 *
 ...
 *
 * Usage:
 *   LIVE_QA=1 SUPABASE_URL=xxx SUPABASE_SERVICE_ROLE_KEY=xxx npx tsx qa/scripts/qa_partner_enrich_e2e.ts
 */

import { dispatchPartnerEnrich } from "./smoke_partner_enrich";

// ── Opt-in guard ──
if (process.env.LIVE_QA !== "1") {
  console.log(
    "⚠️  This is an opt-in live E2E test. Set LIVE_QA=1 to run.\n" +
    "   Example: LIVE_QA=1 SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... npx tsx qa/scripts/qa_partner_enrich_e2e.ts",
  );
  process.exit(0);
}
...
async function main() {
  ...
  const result = await dispatchPartnerEnrich(SUPABASE_URL!, SUPABASE_SERVICE_ROLE_KEY!);
  ...
  // ── C) Validate org_extractions ──
  console.log("═══════════════════════════════════════");
  console.log("  Step C: Validating org_extractions");
  console.log("═══════════════════════════════════════\n");
  ...
  if (!extractions.length) {
    console.error("   ❌ No org_extractions rows found for this run_id");
    process.exit(1);
  }

  console.log(`   ✅ ${extractions.length} org_extractions row(s)`);
  for (const row of extractions) {
    console.log(`      id=${row.id}  org_name=${row.org_name}  created_at=${row.created_at}`);
  }
