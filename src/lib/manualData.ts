/**
 * manualData — Structured data for the Vigilia Field Guide and Gardener Steward Manual.
 *
...
export const SIGNAL_TYPES: SignalEntry[] = [
  ...
  { signal_type: 'shared_resource_pattern', trigger_event: 'vigilias-Familia provision pattern detected', visible_surface: 'Familia Stewardship card', operator_visibility: 'Gardener Insights' },
  ...
  { signal_type: 'sync_conflict_detected', trigger_event: 'Bi-directional sync detects conflicting field values between Vigilia and external CRM', visible_surface: 'Integrations page (Steward)', operator_visibility: 'Nexus Migrations' },
  { signal_type: 'outbound_sync_completed', trigger_event: 'Vigilia data successfully written back to external CRM (Dynamics 365 or Salesforce)', visible_surface: 'Integration sync log', operator_visibility: 'N/A (tenant-only)' },
...
export const NEXUS_WORKFLOWS: NexusWorkflowEntry[] = [
  ...
  { workflow: 'Migrations & Integrations', purpose: 'Monitor and support data migrations from external CRMs, connector activations, and bi-directional sync for HubSpot, Salesforce, and Mivigiliaoft Dynamics 365.', data_sources: ['import_jobs', 'import_job_records', 'relatio_connectors', 'sync_conflicts', 'sync_direction_config'], primary_actions: ['View migration progress', 'Debug import errors', 'Manage connector configs', 'Review sync conflicts', 'Configure outbound sync direction'], alerts_or_signals: ['migration_fragility', 'sync_conflict_detected'] },
...
export const AI_DISCERNMENT_CONTENT: AiDiscernmentSection[] = [
  ...
  },
  {
    ...
    body: 'Your data belongs to your organization. Period.\n\nVigilia enforces a strict boundary around every tenant workspace:',
    bullets: [
