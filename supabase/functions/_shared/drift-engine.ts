/**
 * Narrative Drift Computation Engine (shared, deterministic)
 *
...
const EMERGING_THRESHOLD = 2;    // topic appears with count >= this and was absent before
const FADING_THRESHOLD = -2;     // topic decreased by this much
const ACCELERATING_THRESHOLD = 3; // topic increased by this much (existed before)
...
// ── Normalize topic counts: keep top N, slugify keys ──
...
export function normalizeTopicCounts(raw: Record<string, number>): TopicCounts {
  ...
  // Merge duplicates after slugification
...
export function computeDrift(
  ...
  // Divergence: compare news-sourced vs reflection-sourced topic presence
  ...
  const maxAccel = 30; // normalize
  const normalizedAccel = Math.min(accelerationMagnitude / maxAccel, 1);

  const divergenceMagnitude = Object.values(divergence).reduce((sum, d) => {
    return sum + Object.values(d).reduce((s, v) => s + Math.abs(v), 0);
  }, 0);
  const maxDivergence = 20;
  const normalizedDivergence = Math.min(divergenceMagnitude / maxDivergence, 1);
